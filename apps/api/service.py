    ...
    with _session_scope() as session:
        for batch in rows:
            paper_count = session.scalar(
                select(func.count(CanonicalBatchMembership.paper_id)).where(CanonicalBatchMembership.canonical_batch_id == batch.id)
            ) or 0
            new_count = session.scalar(
                select(func.count(ObservationEvent.id))
                .join(Paper, ObservationEvent.paper_id == Paper.id)
                .where(
                    and_(
                        Paper.canonical_batch_id == batch.id,
                        ObservationEvent.discovered_in_run.is_(True),
                    )
                )
            ) or 0
            updated_count = session.scalar(
                select(func.count(ObservationEvent.id))
                .join(Paper, ObservationEvent.paper_id == Paper.id)
                .where(
                    and_(
                        Paper.canonical_batch_id == batch.id,
                        ObservationEvent.discovered_in_run.is_(False),
                    )
                )
            ) or 0
...
def get_batch_detail(batch_id: str) -> dict[str, Any]:
    ...
    with _session_scope() as session:
        ...
        paper_count = session.scalar(
            select(func.count(CanonicalBatchMembership.paper_id)).where(CanonicalBatchMembership.canonical_batch_id == batch_uuid)
        ) or 0
        new_count = session.scalar(
            select(func.count(ObservationEvent.id))
            .join(Paper, ObservationEvent.paper_id == Paper.id)
            .where(
                and_(
                    Paper.canonical_batch_id == batch_uuid,
                    ObservationEvent.discovered_in_run.is_(True),
                )
            )
        ) or 0
        ...
        papers = session.scalars(
            select(Paper)
            .join(CanonicalBatchMembership, CanonicalBatchMembership.paper_id == Paper.id)
            .where(CanonicalBatchMembership.canonical_batch_id == batch_uuid)
            .order_by(Paper.pub_date.desc(), Paper.title.asc())
        ).all()
